...
public class ShoppingList implements Serializable{
	...
	public ShoppingList(ShoppingListParser listParser) {
		...
		if(listParser.getItems() != null){
			this.items = new Gson().fromJson(listParser.getItems(), new TypeToken<ArrayList<ShoppingListItem>>() {}.getType());
		}
	...
	public ShoppingList(String objectId, String title,
			String items, double total,
			double remaining, String outstandingItem, Date updatedDate, Date createdDate) {
		this.objectId = objectId;
		this.title = title;
		if(items != null){
			this.items = new Gson().fromJson(items, new TypeToken<ArrayList<ShoppingListItem>>() {}.getType());
		}
		this.onwerId = ParseUser.getCurrentUser().getObjectId();
		this.total = total;
	...
	public void setRemaining() {
		...
		for (int i = 0; i < items.size(); i++) {
			if(items.get(i).isChecked())
				remaining -= (items.get(i).getUnitPrice() * items.get(i).getQuantity());
		}
		this.remaining = remaining;
	}
