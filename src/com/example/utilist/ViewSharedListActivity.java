...
public class ViewSharedListActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		//textViewOut = (TextView) findViewById(R.id.view_shared_list_textView_out);
		listView = (ListView) findViewById(R.id.view_shared_list_listView_listView);
		index = getIntent().getIntExtra(SharedListByFriendActivity.SHARELIST_INDEX_KEY, 0);
		sharedLists = (ArrayList<SharedList>) getIntent().getSerializableExtra(SharedListByFriendActivity.SHARELIST_KEY);
		...
		textViewListTitle.setText("List title: " + sharedLists.get(index).getListTitle());
		sharedLists.get(index).setNewList(false);
		SharedListParser sharedListParser = new SharedListParser(sharedLists.get(index));
	...
	public void showTodoList(){
		...
		parseQuery.whereEqualTo("objectId", sharedLists.get(index).getListObjectId());
	...
	public void showTodoListItem(){
		textViewNumberOfItems.setText("# of items: " + todoList.getItems().size());
//		textViewOut.setText(todoList.getOutstandingItem());
		TodoListDisplayItemAdapter adapter = new TodoListDisplayItemAdapter(ViewSharedListActivity.this, R.layout.activity_todo_list_display_item_layout, todoList.getItems(), null){
	...
	public void showShoppingListItem(){
		textViewNumberOfItems.setText("# of items: " + shoppingList.getItems().size());
//		textViewOut.setText(shoppingList.getOutstandingItem());
		ShoppingListDisplayItemAdapter adapter = new ShoppingListDisplayItemAdapter(ViewSharedListActivity.this, R.layout.display_item_layout, shoppingList.getItems(), null){
			public boolean isEnabled(int position) 
            { 
                    return false; 
            } 
		...
		if(shoppingList.getTotal() != 0){
			...
			textViewRemaining.setText("Remaining: " + shoppingList.getRemaining());
