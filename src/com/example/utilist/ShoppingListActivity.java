...
import com.example.adapter.ShoppingListAdapter;
...
//			
//		}
		currShoppingList = (ShoppingList) getIntent().getSerializableExtra(MainActivity.CURR_SHOPPING_LIST_KEY);
		lists = new ArrayList<ShoppingList>();
		//lists = (ArrayList<ShoppingList>)getIntent().getSerializableExtra(MainActivity.SHOPPING_LIST_KEY);
	...
	public void onClick(View v) {
		...
		case R.id.shopping_list_imageViewRight:
			if(mode.equals("adding") && editingModeIndex == -1){
				...
				lists.get(editingModeIndex).setTitle(et_add_shopping_list.getText().toString());
