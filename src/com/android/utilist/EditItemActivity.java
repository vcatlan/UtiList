...
public class EditItemActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		list = (ShoppingList) getIntent().getSerializableExtra(
				MainFragment.ITEM_NAME_KEY);
		index = getIntent().getIntExtra(MainFragment.ITEM_INDEX_KEY,
				0);
		listIndex = getIntent().getIntExtra(
		...
		for (int i = 0; i < getResources().getStringArray(R.array.type).length; i++) {
			if (item.getType().toLowerCase()
					.equals(getResources().getStringArray(R.array.type)[i]))
			...
			public void onClick(View v) {
				if (itemName.getText().toString().equals(""))
					Toast.makeText(EditItemActivity.this,
							"Item's name cannot be empty", Toast.LENGTH_SHORT)
							.show();
				else {
					item.setName(itemName.getText().toString());
					item.setType(itemType.getSelectedItem().toString());
					if (!itemQty.getText().toString().equals(""))
						item.setQuantity((Integer.valueOf(itemQty.getText()
								.toString()) == 0) ? 1 : (Integer
								.valueOf(itemQty.getText().toString())));
					else
						item.setQuantity(1);
					if (!itemPrice.getText().toString().equals(""))
						item.setUnitPrice(Double.valueOf(itemPrice.getText()
								.toString()));
					else
						item.setUnitPrice(0);
					...
					if (!itemPrice.getText().toString().equals("")) {
						if (Double.valueOf(itemPrice.getText().toString()) != 0)
							history.getPriceHistoryList().add(
		...
		priceHistory.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				CharSequence[] charSequences = new CharSequence[history
						.getPriceHistoryList().size()];
				AlertDialog.Builder builder = new AlertDialog.Builder(
						EditItemActivity.this);
				builder.setTitle("Select Price");
				if (charSequences.length != 0)
					builder.setItems(history.getPriceCarSequence(),
							new DialogInterface.OnClickListener() {

								@Override
								public void onClick(DialogInterface dialog,
										int which) {
									itemPrice.setText(history
											.getPriceCarSequence()[which]);
