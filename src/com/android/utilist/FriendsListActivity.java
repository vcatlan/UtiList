...
import com.android.adapter.DisplayFriendAdapter;
import com.android.model.FriendList;
import com.android.model.FriendListItem;
...
public class FriendsListActivity extends Activity implements
		DisplayFriendAdapter.ICreateCallToFriendListItemClick {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		ParseQuery<SharedListParser> parseQuerySharedList = new ParseQuery<SharedListParser>(
				SharedListParser.class);
		parseQuerySharedList.whereEqualTo("sharedByObjectId", ParseUser
				.getCurrentUser().getObjectId());
		parseQuerySharedList
				.findInBackground(new FindCallback<SharedListParser>() {
					@Override
					public void done(List<SharedListParser> objects,
							ParseException e) {
						if (e == null) {
	...
	public void showFriendsList() {
		...
		adapter = new DisplayFriendAdapter(this,
				R.layout.display_friend_layout, list.getItems(),
														...
														if (e == null) {
															if (objects.size() != 0) {
																if (!list
																	...
																	Toast.makeText(
																			getApplicationContext(),
																			"The new friend is aleardy in your list!",
																			Toast.LENGTH_SHORT)
																			.show();
																}
															} else {
	...
	public void shareTodoList() {
		...
		for (Integer t : selectedTodoList) {
			for (Integer f : selectedFriends) {
				SharedList sharedList = new SharedList(todoLists.get(t)
						.getObjectId(), user.getObjectId(), todoLists.get(t)
						.getTitle(), user.getString("firstName") + " "
						+ user.getString("lastName"), true, "todoList", list
						.getItems().get(f).getObjectId());
		...
		ParseObject.saveAllInBackground(newSharedListsParsers,
				new SaveCallback() {
					@Override
					public void done(ParseException e) {
						if (e == null) {
							ParseQuery<SharedListParser> parseQuerySharedList = new ParseQuery<SharedListParser>(
									SharedListParser.class);
							parseQuerySharedList.whereEqualTo(
									"sharedByObjectId", ParseUser
											.getCurrentUser().getObjectId());
							parseQuerySharedList
									.findInBackground(new FindCallback<SharedListParser>() {
										@Override
										public void done(
												List<SharedListParser> objects,
												ParseException e) {
											if (e == null) {
												sharedLists = new ArrayList<SharedList>();
												for (SharedListParser s : objects) {
	...
	public void shareShoppingList() {
		...
		for (Integer t : selectedShoppingList) {
			for (Integer f : selectedFriends) {
				SharedList sharedList = new SharedList(shoppingLists.get(t)
						.getObjectId(), user.getObjectId(), shoppingLists
						.get(t).getTitle(), user.getString("firstName") + " "
						+ user.getString("lastName"), true, "shoppingList",
						list.getItems().get(f).getObjectId());
				if (sharedLists.indexOf(sharedList) == -1) {
					newSharedListsParsers.add(new SharedListParser(sharedList));
				}
			}
	...
	public void checkBokOnClick(View v) {
		if (((CheckBox) v).isChecked() == true)
			selectedFriends.add(Integer.parseInt(v.getTag()
					.toString()));
		else if (((CheckBox) v).isChecked() == false)
			selectedFriends.remove((Integer) Integer.parseInt(v.getTag()
					.toString()));
