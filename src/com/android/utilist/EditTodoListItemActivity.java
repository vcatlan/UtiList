...
public class EditTodoListItemActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		list = (TodoList) getIntent().getSerializableExtra(
				TodoListFragment.ITEM_NAME_KEY);
		history = (History) getIntent().getSerializableExtra(
				TodoListFragment.ITEM_HISTORY_KEY);
		index = getIntent().getIntExtra(
				TodoListFragment.ITEM_INDEX_KEY, 0);
		...
		save.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				if (itemName.getText().toString().equals(""))
					Toast.makeText(EditTodoListItemActivity.this,
	...
	public void showDatePicker() {
		DatePickerDialog.OnDateSetListener onDateSetListener = new DatePickerDialog.OnDateSetListener() {
			...
			public void onDateSet(DatePicker view, int year, int monthOfYear,
					int dayOfMonth) {
				itemDate.setText((monthOfYear + 1) + "/" + dayOfMonth + "/"
						+ year);
				if (itemTime.getText().toString().equals("...")) {
					String newString = new SimpleDateFormat("h:mm a")
							.format(new Date(System.currentTimeMillis()));
					itemTime.setText(newString);
					// Date date = new
					// SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(originalString);
			...
			public void onTimeSet(TimePicker view, int hourOfDay, int minute) {
				if (hourOfDay >= 12) {
					if (hourOfDay == 12)
						...
						itemTime.setText((hourOfDay - 12) + ":" + minute
								+ " PM");
				} else {
					if (hourOfDay == 0)
						itemTime.setText((hourOfDay + 12) + ":" + minute
				...
				if (itemDate.getText().toString().equals("...")) {
					String newString = new SimpleDateFormat("mm/dd/yyyy")
							.format(new Date(System.currentTimeMillis()));
